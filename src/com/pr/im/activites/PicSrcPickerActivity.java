...
//	public static final int TAKE_PIC = 1;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null)  ////判断 activity被销毁后 有没有数据被保存下来
		{
			...
			video_Name = "t"+ String.valueOf(System.currentTimeMillis()).substring(5)+".3gp"; 
...
//				break;
			case 1:
				...
				if (!TextUtils.isEmpty(uri.getAuthority())) {
					Cursor cursor = getContentResolver().query(uri,
							...
							null, null, null);
					if (null == cursor) {
						Tool.initToast(getApplicationContext(), "视频没找到");
						return;
					}
					cursor.moveToFirst();
					...
					if (data != null) {
						...
						try {
							Intent picIntent = new Intent();
							picIntent.putExtra("imgName", video_Name);
							picIntent.putExtra("base64String", ImageUtil.getBase64StringFromFile(path));
							picIntent.putExtra("imgPath", path);
        ...
        else if (resultCode == RESULT_CANCELED && (type == CHOOSE_VIDEO || type == CHOSE_PIC)) {
        	finish();
		}
    }
